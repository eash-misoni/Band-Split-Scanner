...
現在は、任意の帯数を扱うための基本編集機能、細部を調整するためのズーム・パン操作、補正結果画面の出力境界線表示切り替えまで実装済みである。

次の主な作業対象は、補正結果画面の出力外枠、出力縦横比の変更、保存処理などである。
...
補正結果画面へ出力外枠を追加し、補正後画像全体の出力範囲を視覚化する。

MVPでは、右辺または下辺のドラッグによって出力サイズまたは縦横比を変更できる構成から実装する。

出力境界線の表示切り替えは実装済みであるため、次は補正結果画面上の編集対象を内部境界から画像全体の外枠へ広げる。

\---

### 候補2: 出力縦横比の変更

出力外枠の操作結果を `OutputSettings` へ反映し、補正後画像全体の幅・高さまたは縦横比を変更できるようにする。
...
出力境界の `outputX` と、画像全体の出力サイズを別の状態として扱う必要がある。
...
出力境界線の表示切り替えまで実装できたため、次は補正結果画面上に画像全体の出力範囲を示す外枠を追加する。

まずは外枠の表示だけを実装し、その後に右辺または下辺のドラッグと `OutputSettings` の更新へ段階的に進む。
...
現在は、帯分割補正の基本検証に加えて、任意の境界数を編集するための追加・移動・削除操作、ズーム・パンによる詳細編集操作、補正結果画面の出力境界線表示切り替えまで実装済みである。

次の段階では、補正結果画面の出力外枠、出力縦横比の変更、保存処理などの機能拡張へ進む。
