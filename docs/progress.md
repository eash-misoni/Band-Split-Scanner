...
- 補正結果画面から現在のcorrectedBitmapをJPEGとして保存できる
...
現在は、任意の帯数を扱うための基本編集機能、補正前・補正後画面のズーム・パンと最低48dp可視制約、補正結果画面の出力境界線表示切り替え、View端に依存しない出力外枠の左右辺ドラッグによる縦横比変更に加え、現在のプレビュー用補正BitmapをMediaStoreへ保存する最小経路まで実装済みである。

次の主な作業対象は、保存時に編集状態から保存用Bitmapを再生成する処理、プレビュー用画像と保存用高解像度画像の分離、画像入出力責務と状態管理の整理などである。
...
保存ボタンを押す
↓
現在のcorrectedBitmapをJPEGとしてMediaStoreへ保存
↓
...
現在の最小実装では、プレビュー表示に使用している `correctedBitmap` をそのままJPEGへ圧縮し、Androidの `MediaStore` へ保存する。
...
現時点の保存画像は、幅 `1200px` で生成したプレビュー用 `correctedBitmap` である。元画像から保存用高解像度Bitmapを再生成する処理は未実装。
...
現在の最小保存処理も、この `correctedBitmap` を直接JPEGとして保存する。

保存用高解像度画像の再生成とプレビュー用縮小読込を進める際に、URI中心の状態管理へ移行する。
...
```text
- カメラ撮影
- 現在の編集状態から保存用Bitmapを再生成する処理
- 保存時の高解像度再読込
- プレビュー用縮小Bitmapと保存用高解像度Bitmapの分離
...
現在のプレビュー画像保存までは検証版として許容するが、高解像度保存・撮影・画面分割へ進む段階では整理が必要。
...
現在の保存処理は、画面確認用に幅 `1200px` で生成した `correctedBitmap` をそのまま保存している。

次は、保存ボタンを押した時点で現在の編集状態を取得し、保存用の `OutputSettings` で補正結果を再生成してから保存する。
...
保存用出力幅の決定方法と、端末メモリを超えない上限をあわせて検討する。

\---

### 候補2: プレビュー用画像と保存用高解像度画像の分離

現在は、選択画像を `sourceBitmap` として直接読み込み、表示・プレビュー補正・保存用補正の入力として共用している。

将来は次の構成へ分離する。

```text
プレビュー
→ selectedImageUriから縮小Bitmapを読み込む
→ 軽量な補正プレビューを生成

保存
→ selectedImageUriから保存用解像度で再読込
→ 同じ編集状態を保存用画像座標へ変換
→ 高解像度の最終補正画像を生成
```

縮小プレビュー導入時には、`PageCorners` と `BoundaryPair.inputTop` / `inputBottom` をどの座標系で正本化するかを先に決める必要がある。

\---

### 候補3: 画像入出力と状態管理の整理

現在の保存処理は `MainActivity` が直接担当している。

高解像度保存へ進む前後で、次を段階的に分離する。

```text
ImageLoader
= URIからプレビュー用・保存用Bitmapを読み込む

ImageSaver
= BitmapをMediaStoreへ保存する

編集状態
= PageCorners / BoundaryPair / outputAspectRatioの正本
```
...
```text
保存ボタン押下時に、現在の編集状態から保存用Bitmapを再生成する
```

MediaStoreへ保存する経路と、保存画像へ編集用オーバーレイを含めない処理は動作確認済みである。

次は、プレビュー用 `correctedBitmap` の直接保存をやめ、次の経路へ変更する。
...
この段階では、まず現在保持している `sourceBitmap` を入力として保存用Bitmap再生成を確認する。

その後、`selectedImageUri` の保持、プレビュー用縮小読込、保存用高解像度再読込、画像座標の変換へ進む。
...
現在は、帯分割補正の基本検証に加えて、任意の境界数を編集するための追加・移動・削除操作、補正前・補正後画面のズーム・パンと最低48dp可視制約、出力境界線表示切り替え、View端に依存しない出力外枠操作、プレビュー用補正画像のMediaStore保存まで実装済みである。

次の段階では、現在の編集状態から保存用Bitmapを再生成する処理、プレビュー用画像と保存用高解像度画像の分離、画像入出力責務と状態管理の整理などへ進む。
