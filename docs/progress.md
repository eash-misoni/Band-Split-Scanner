...
- 補正結果画面に出力外枠を表示できる
- 出力外枠の右辺ドラッグで出力縦横比を変更できる
- ドラッグ終了時に新しい出力サイズで補正結果を再生成できる
- 元画像編集画面と補正結果画面を往復して再調整できる
```

現在は、任意の帯数を扱うための基本編集機能、元画像側のズーム・パン、補正結果画面の出力境界線表示切り替え、出力外枠の右辺ドラッグによる縦横比変更まで実装済みである。

次の主な作業対象は、出力外枠の左辺ドラッグ、補正結果プレビューのズーム・パン、保存処理などである。
...
補正結果画面へ出力外枠を表示し、右辺ドラッグによる出力縦横比変更を実装済み。
...
現在の実装は右辺のみを操作対象とし、左辺を固定する。

将来は左辺ドラッグにも対応し、右辺を固定して横幅を変更できるようにする。
...
また、`MainActivity` は補正結果画面の表示状態として以下を保持する。
...
この値は表示専用であり、補正結果Bitmapや `BoundaryPair` には影響しない。
...
### 11.6 出力外枠がView端に制限される

現在の右辺ドラッグは、出力外枠の右辺が `ResultPreviewView` の右端を超えないよう制限している。

そのため、画面上の空き領域がなくなると、それ以上横長の縦横比へ変更できない。

将来は補正結果プレビューへズーム・パンを導入し、次の状態を分離する。

```text
outputAspectRatio
= 補正結果の内容と縦横比

previewZoomScale / previewPanX / previewPanY
= 画面上の見え方
```

出力外枠の操作範囲は、View端ではなく最小・最大縦横比によって制限する。
...
左右どちら側からでも、操作しやすい方向へ出力横幅を変更できるようにする。

\---

### 候補2: 補正結果プレビューのズーム・パン

補正結果画面でもピンチズームとパンを可能にする。
...
両者を分離することで、画面端に制限されず出力外枠を広げられるようにする。

\---

### 候補3: 保存処理

現在の編集状態と `OutputSettings` から補正結果を生成し、端末へ保存する経路を実装する。
...
右辺ドラッグによる縦横比変更は動作確認済みであるため、まず同じイベント経路を左辺にも拡張する。

左辺ドラッグでは右辺を固定し、右辺ドラッグでは左辺を固定する。

左右辺操作が揃った後、補正結果プレビューのズーム・パンを実装し、View端に依存しない出力外枠操作へ進む。
...
現在は、帯分割補正の基本検証に加えて、任意の境界数を編集するための追加・移動・削除操作、元画像側のズーム・パン、補正結果画面の出力境界線表示切り替え、出力外枠の右辺ドラッグによる縦横比変更まで実装済みである。

次の段階では、出力外枠の左辺ドラッグ、補正結果プレビューのズーム・パン、保存処理などの機能拡張へ進む。
