...
- 補正結果画面からJPEG画像を保存できる
- 保存時に現在のPageCorners・BoundaryPair・outputAspectRatioから保存用Bitmapを再生成できる
- 保存用Bitmapと幅1200pxのプレビュー用correctedBitmapを分離できる
- 保存幅をページ上辺・下辺の平均幅から決定できる
...
- 保存後に一時的な保存用Bitmapを解放できる
```

現在は、任意の帯数を扱うための基本編集機能、補正前・補正後画面のズーム・パンと最低48dp可視制約、補正結果画面の出力境界線表示切り替え、View端に依存しない出力外枠の左右辺ドラッグによる縦横比変更に加え、現在の編集状態から保存用Bitmapを再生成してMediaStoreへ保存する経路まで実装済みである。

次の主な作業対象は、`selectedImageUri` の保持、プレビュー用縮小Bitmapと保存用再読込Bitmapの分離、画像座標の変換方針、画像入出力責務と状態管理の整理などである。
...
保存ボタンを押す
↓
現在のPageCorners・BoundaryPair・outputAspectRatioを取得
↓
...
### 5.17 補正結果の保存用Bitmap再生成

補正結果画面に保存ボタンを実装済み。

保存時は、幅 `1200px` のプレビュー用 `correctedBitmap` を直接保存せず、現在の編集状態から保存用Bitmapを別途再生成する。
...
保存用の出力幅は、元画像座標上のページ上辺と下辺の長さの平均から決める。
...
現行の `ScanlineBandRenderer` は元画像と出力画像の画素配列をメモリ上へ展開するため、保存用出力サイズには次の上限を設けている。
...
保存用Bitmapを生成
↓
JPEG品質95でMediaStoreへ保存
↓
保存完了Toastに実際の出力サイズを表示
↓
保存用Bitmapをrecycle
```
...
Android 10以降では、保存先として次を指定する。

```text
Pictures/BandSplitScanner
```

Android 9以前では、保存前に `WRITE_EXTERNAL_STORAGE` 権限を確認し、未許可の場合はActivity Result APIで権限を要求する。

保存処理ではBitmap本体だけをJPEGへ圧縮するため、以下の編集補助用表示は保存画像へ含まれない。
...
保存中は保存ボタンを一時的に無効化し、処理終了後に再度有効化する。保存に失敗した場合は、途中まで登録したMediaStore項目を削除する。

`OutOfMemoryError` は個別に捕捉し、保存画像生成時のメモリ不足として通知する。

現時点では、画像選択時に読み込んだ `sourceBitmap` を保存用補正の入力として使用している。URIから保存用解像度で再読込する処理は未実装。
...
- 現在の編集状態から保存用Bitmapを再生成
- ページ上辺・下辺の平均幅による保存用出力幅の決定
- 最大400万画素・長辺4096pxによる保存サイズ制限
- 保存用BitmapのJPEG圧縮
- MediaStoreへの保存と公開
- 保存成功・失敗・メモリ不足メッセージの表示
- 保存完了時の実出力サイズ表示
- 保存後の一時Bitmap解放
```
...
保存時には、現在の編集状態と `sourceBitmap` から保存用Bitmapを一時生成する。
...
`saveBitmap` は保存後に解放する。

ただし、`sourceBitmap` は画像選択時にURIから一度だけ読み込んだBitmapであり、保存時にURIから再読込しているわけではない。

プレビュー用縮小読込と保存用再読込を進める際に、URI中心の状態管理へ移行する。
...
- selectedImageUriの保持
- 保存時のURIからの再読込
- プレビュー用縮小Bitmapと保存用再読込Bitmapの分離
- プレビュー画像座標と原画像座標の変換
- ImageLoader / ImageSaverへの画像入出力責務分離
- 保存処理のバックグラウンド実行
- 共有
```
...
現在の保存用Bitmap再生成までは検証版として許容するが、URI再読込・バックグラウンド処理・撮影・画面分割へ進む段階では整理が必要。
...
保存用Bitmapは最大400万画素・長辺4096pxへ制限しているが、入力側の `sourceBitmap` と、`ScanlineBandRenderer` が使用する画素配列のメモリ負荷は残る。

スマートフォン写真の解像度によってはメモリ負荷が大きくなるため、将来的にはプレビュー用縮小読込と保存用再読込を分離する。

\---

### 11.4 保存処理のUIスレッド実行

現在の保存用Bitmap生成、JPEG圧縮、MediaStore保存は、保存ボタンのイベントから同期的に実行している。

保存画像が大きい場合は操作が一時的に停止する可能性がある。

画像入出力責務を分離する段階で、バックグラウンドスレッドまたは非同期処理へ移す。
...
### 候補1: selectedImageUriの保持と保存時再読込

現在は、画像選択時にURIから `sourceBitmap` を一度だけ読み込み、そのBitmapを編集表示・プレビュー補正・保存用補正で共用している。

次は、選択したURIを `MainActivity` で保持し、保存時にURIからBitmapを再読込できる入口を作る。
...
保存ボタン
↓
selectedImageUriから保存用Bitmapを再読込
↓
現在の編集状態で最終補正
```

最初の小さい実装単位では、現在の読込方法を大きく変えず、URI保持と保存時再読込の経路だけを追加する。

\---

### 候補2: プレビュー用縮小Bitmapの導入

URI保持と保存時再読込ができた後、編集画面では縮小したBitmapを使用する。
...
保存
→ URIからより大きなBitmapを再読込
```

この変更では、`PageCorners` と `BoundaryPair.inputTop` / `inputBottom` をプレビュー画像座標で保持するか、原画像に対する正規化座標で保持するかを決める必要がある。

\---

### 候補3: ImageLoader / ImageSaverへの責務分離

現在は読込・補正・保存が `MainActivity` に集中している。

URI再読込と縮小読込を実装する段階で、次を分離する。

```text
ImageLoader
= URIから指定条件でBitmapを読み込む

ImageSaver
= BitmapをMediaStoreへ保存する

MainActivity
= 処理の開始と画面状態管理
```

保存処理のバックグラウンド実行も、この分離とあわせて検討する。
...
```text
選択した画像URIを保持し、保存時にURIから再読込できる経路を追加する
```

現在は、保存ボタン押下時に次の処理まで動作確認済みである。
...
次は、保存用補正の入力を常時保持している `sourceBitmap` だけに依存しない形へ進める。
...
ただし、プレビュー用縮小Bitmapを同時に導入すると座標変換まで変更範囲が広がる。

そのため、次の実装ではまず以下に限定する。

```text
- imagePickerLauncherで選択URIを保持する
- 保存時に同じURIからBitmapを再読込する
- 現在と同じ画像座標で保存結果が一致することを確認する
```

縮小プレビューと原画像座標への変換は、その次の作業とする。
...
- ページ上辺・下辺の平均幅による保存用出力幅決定
- 保存画像の最大400万画素・長辺4096px制限
- 保存用BitmapのJPEG保存
- 保存完了時の実出力サイズ表示
- 保存後の一時Bitmap解放
- OutOfMemoryErrorの個別通知
- Android 10以降のPictures/BandSplitScannerへのMediaStore保存
- Android 9以前のWRITE_EXTERNAL_STORAGE権限要求
- 保存画像から編集用オーバーレイを除外
- 保存失敗時のMediaStore登録削除
```

現在は、帯分割補正の基本検証に加えて、任意の境界数を編集するための追加・移動・削除操作、補正前・補正後画面のズーム・パンと最低48dp可視制約、出力境界線表示切り替え、View端に依存しない出力外枠操作、現在の編集状態からの保存用Bitmap再生成とMediaStore保存まで実装済みである。

次の段階では、`selectedImageUri` の保持、保存時のURI再読込、プレビュー用縮小Bitmapと保存用Bitmapの分離、画像座標変換、画像入出力責務と状態管理の整理などへ進む。
