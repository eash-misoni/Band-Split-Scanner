...
現在は、任意の帯数を扱うための基本編集機能、元画像側のズーム・パン、補正結果画面の出力境界線表示切り替え、出力外枠の左右辺ドラッグによる縦横比変更まで実装済みである。

次の主な作業対象は、補正結果プレビューのズーム・パン、View端に依存しない出力外枠操作、保存処理などである。
...
現在の左右辺ドラッグは、操作対象の辺が `ResultPreviewView` の端を超えないよう制限している。
...
### 候補2: View端に依存しない出力外枠操作

現在は、左右辺のドラッグ位置を `ResultPreviewView` の端で制限している。

補正結果プレビューのズーム・パン導入後は、出力外枠の操作範囲をView端ではなく、最小・最大縦横比だけで制限する。
...
出力外枠の左右辺ドラッグによる縦横比変更は動作確認済みである。

次は、補正結果画面にも表示専用のズーム・パン状態を追加し、出力縦横比と画面上の見え方を分離する。

これにより、出力外枠がView端へ到達した場合でも、プレビューを縮小またはパンして操作領域を確保できるようにする。
...
現在は、帯分割補正の基本検証に加えて、任意の境界数を編集するための追加・移動・削除操作、元画像側のズーム・パン、補正結果画面の出力境界線表示切り替え、出力外枠の左右辺ドラッグによる縦横比変更まで実装済みである。

次の段階では、補正結果プレビューのズーム・パン、View端に依存しない出力外枠操作、保存処理などの機能拡張へ進む。
