...
現行実装では、画像がViewを覆う範囲を中心とした比較的厳しいパン制約を使用している。

次の変更では、パン時とピンチ時の両方について、画像を横方向・縦方向それぞれ最低 `48dp` だけ画面内へ残す制約へ変更する。
...
現在の `CornerEditView` は、パン後の画像をView内へ比較的強く留める制約を使用している。

新しい仕様では、パン操作とピンチ操作のどちらでも、画像が横方向・縦方向のそれぞれで最低 `48dp` 見えていればよい。
...
現行実装の `constrainPan()` は、この共通ルールへ変更する必要がある。

\---

### 11.7 出力外枠がView端に制限される
...
補正結果画面でも、パンとピンチ後に補正後画像を縦横それぞれ最低 `48dp` 画面内へ残す。
...
`CornerEditView` の現行パン制約を変更する。

```text
パン操作後
→ 画像を縦横それぞれ最低48dp画面内へ残す
...
パンとピンチで別の制約を使わず、変換後の画像表示矩形に対して同じ処理を適用する。
...
補正後Bitmap、出力境界線、出力外枠へ共通の表示行列を適用する。

パンとピンチ後は、補正後画像を縦横それぞれ最低 `48dp` 画面内へ残す。
...
補正前画面にはすでにズーム・パンが実装されているため、まず既存の表示位置制約だけを新仕様へ差し替える。

その後、同じ制約ロジックを基準に `ResultPreviewView` へズーム・パンを実装する。
...
次の段階では、補正前画面の48dp可視制約への変更、補正結果プレビューのズーム・パン、View端に依存しない出力外枠操作、保存処理などの機能拡張へ進む。
