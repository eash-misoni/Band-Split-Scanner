...
現在は、固定帯数による補正方式の検証を終え、本格MVPの主要な編集・プレビュー・保存経路を実装し、画像入出力責務の分離まで進んだ段階である。
...
- 元画像編集画面と補正結果画面をズーム・パンできる
- 両画面で画像を縦横それぞれ最低48dp画面内へ残せる
- 補正結果画面で出力境界線の表示・非表示を切り替えられる
- 出力外枠の左右辺ドラッグで出力縦横比を変更できる
...
- ImageLoaderへ画像寸法取得・縮小読込・原画像読込を分離できる
- ImageSaverへJPEG圧縮・MediaStore登録・公開・失敗時削除を分離できる
```

画像選択から入力境界編集、補正結果確認、出力幅・縦横比調整、原画像再読込による保存までの主要操作は実装済みである。

画像読込とMediaStore保存は専用クラスへ分離済みであり、次の主な作業対象は、`PageCorners` と `BoundaryPair` をCustom Viewの外へ移す編集状態管理、画面回転やActivity再生成を考慮したライフサイクル対応である。
...
最初は `MainActivity` が保持し、`CornerEditView` との受け渡し方法を整理する。

\---

### 候補2: PageCornersとBoundaryPairのView外への移動

`CornerEditView` を編集状態の正本ではなく、描画・操作入力を担当するViewへ近づける。

```text
MainActivityまたはViewModel
↓ 最新の編集状態を渡す
CornerEditView
↓ 操作イベントを通知
MainActivityまたはViewModel
...
一度にすべて移さず、まずページ基準点または内部境界の片方から段階的に移行する。

\---

### 候補3: 保存ジョブのライフサイクル整理

編集状態の所有者を整理した後、保存処理の所有者もActivity外へ移す。
...
```text
ScanEditStateを追加し、PageCorners・BoundaryPair・outputAspectRatioをまとめる
```

`ImageLoader` と `ImageSaver` の責務分離は動作確認済みである。

次の実装では、まだViewModelやFragmentを導入せず、以下に限定する。

```text
- model/ScanEditState.javaを追加する
- MainActivityがScanEditStateを保持する
- 現在の編集状態をScanEditStateへ同期する入口を作る
- 画面の動作は変えない
```

その後、`CornerEditView` が保持する状態を段階的に `ScanEditState` 側へ移す。
...
画像選択から帯分割編集、補正結果確認、幅・縦横比調整、高解像度入力を使った保存まで、MVPの主要な一連操作は動作している。

画像入出力責務の分離も完了した。

次の段階では、`ScanEditState` を導入し、`PageCorners` と `BoundaryPair` の正本をCustom Viewの外へ移す準備を進める。
