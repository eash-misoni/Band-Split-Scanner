...
現在は、任意の帯数を扱うための基本編集機能まで実装済みである。

次の主な作業対象は、元画像編集画面のズーム・パン、補正結果画面の操作機能、保存処理などである。
...
境界の追加・削除・移動による基本編集サイクルは成立したため、細かい位置調整を行えるよう、元画像編集画面へズーム・パンを追加する。
...
`ResultPreviewView` には出力境界線を表示する機能があるため、UI側にボタンまたはスイッチを追加する。
...
境界の追加・削除・移動による任意帯数の基本編集が可能になったため、次は細かい境界位置を調整するための表示操作を実装する。
...
現在は、帯分割補正の基本検証に加えて、任意の境界数を編集するための追加・移動・削除操作まで実装済みである。

次の段階では、ズーム・パンによる詳細編集操作と、補正結果画面の機能拡張へ進む。
