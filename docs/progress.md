...
現在は、任意の帯数を扱うための基本編集機能、元画像側のズーム・パンと最低48dp可視制約、補正結果画面の出力境界線表示切り替え、出力外枠の左右辺ドラッグによる縦横比変更まで実装済みである。
...
```text
補正結果プレビューへズーム・パンを実装する
```

`CornerEditView` では、パン・ピンチ後に画像を縦横それぞれ最低48dp残す表示位置制約を実装済みである。

次は、この考え方を基準として `ResultPreviewView` に以下を導入する。

```text
- 補正後Bitmapへ適用する共通表示行列
- 出力境界線と出力外枠への同じ表示変換
- 空き領域の1本指パン
- ピンチ中心を維持する2本指ズーム
- パン・ピンチ後の最低48dp可視制約
...
補正結果プレビューのズーム・パンを実装した後、出力外枠の操作範囲をView端から切り離す。
...
現在は、帯分割補正の基本検証に加えて、任意の境界数を編集するための追加・移動・削除操作、元画像側のズーム・パンと最低48dp可視制約、補正結果画面の出力境界線表示切り替え、出力外枠の左右辺ドラッグによる縦横比変更まで実装済みである。
