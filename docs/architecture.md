...
将来の補正結果ズーム・パンでは、以下の状態を出力設定から分離する。
...
現在のドラッグ可能範囲は、縦横比制約に加えてView端でも制限している。将来は補正結果プレビューのズーム・パンを導入し、View端への依存をなくす。

---

### 7.9 補正結果プレビューのズーム・パン

将来、補正結果画面にも表示専用のズーム・パン状態を追加する。
...
出力外枠編集とプレビュー表示変換を分離することで、画面端に依存せず出力外枠を拡張できるようにする。
...
境界追加・削除、元画像側のズーム・パン、出力境界線表示切り替え、出力外枠表示、左右辺ドラッグによる縦横比変更は実装済み。

残る主な作業は以下。

```text
- 補正前画面のパン・ピンチ制約を最低48dp可視方式へ変更
- 補正結果プレビューのズーム・パン
- 補正結果画面への最低48dp可視制約
- View端に依存しない出力外枠操作
- 保存
...
- 出力縦横比と補正結果プレビューの表示状態を分離できる構造になっている
- 補正前・補正後のパンとピンチに共通の最低48dp可視制約を適用する方針が定義されている
```

今後の大きな設計課題は、MainActivityとCornerEditViewに集中している状態管理を、機能拡張のタイミングで段階的に外へ出すことである。

現段階では、最終構成への一括リファクタリングより、補正結果プレビューのズーム・パン、View端に依存しない出力外枠操作、保存といった残りのMVP機能を検証しながら、必要な単位で責務分離を進める。
