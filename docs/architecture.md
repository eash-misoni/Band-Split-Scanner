...
将来の左辺ドラッグでも同じイベント経路を使い、固定する辺だけを切り替える。

出力外枠のドラッグ可能範囲は、Viewの端ではなく縦横比制約で決める。
...
境界追加・削除、元画像側のズーム・パン、出力境界線表示切り替え、出力外枠表示、右辺ドラッグによる縦横比変更は実装済み。
...
現段階では、最終構成への一括リファクタリングより、出力外枠の左右辺操作、補正結果プレビューのズーム・パン、保存といった残りのMVP機能を検証しながら、必要な単位で責務分離を進める。
